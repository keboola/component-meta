...
INSTAGRAM_ACCOUNTS = {
    "177057932317550": {
        "instagram_business_account": {"id": "17841403584244541"},
        "name": "Keboola",
...
def get_replacement_id(category: str, original_id: str, ads_accounts: List[str]) -> str:
    ...
    if category == "ads":
        ...
        ig_accounts = [
            acc
            for acc in INSTAGRAM_ACCOUNTS.values()
...
def determine_required_id_type(
    query_obj: Dict[str, Any], component_category: str
) -> str:
    """
    ...
    if "insights" in query_str:
        ...
        if (
            "action_type" in query_str
            or "breakdown" in query_str
            or "action_breakdown" in query_str
        ):
            return "ads"
...
def sanitize_query_json(
    query_obj: Dict[str, Any], category: str, ads_accounts: List[str]
) -> Dict[str, Any]:
    """
